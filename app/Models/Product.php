...
class Product extends Model
{
    ...
    public static function getDiscountPrice($product_id) { // this method is called in front/index.blade.php
        ...
        $productDetails = \App\Models\Product::select('product_price', 'product_discount', 'category_id', 'price_per_pax', 'pax')->where('id', $product_id)->first();
        ...
        if ($productDetails['product_discount'] > 0) { // if there's a 'product_discount' (in `products` table) (i.e. discount is not zero 0)
            ...
            $discounted_price = $orig_price - ($orig_price * $discount_percentage);
            ...
            $orig_price = $productDetails['price_per_pax'] * $productDetails['pax'];
            $discount_percentage = $categoryDetails['category_discount'] / 100;
            $discounted_price = $orig_price - ($orig_price * $discount_percentage);
    ...
    public static function getDiscountAttributePrice($product_id, $size) { // this method is called (used) in front/products/detail.blade.php and cart_items.blade.php and in applyCoupon() method in Front/ProudctsController.php
        ...
        if ($proDetails['product_discount'] > 0) { // if there's a 'product_discount' (in `products` table) (i.e. discount is not zero 0)
            ...
            $orig_price = $product['price_per_pax'] * $size;
            $discount_percentage = $product['product_discount'] / 100;
            $discount = $orig_price * $discount_percentage;
            $final_price = $orig_price - $discount;
            ...
            $orig_price = $product['price_per_pax'] * $size;
            $discount_percentage = $catDetails['category_discount'] / 100;
            $discount = $orig_price * $discount_percentage;
            $final_price = $orig_price - $discount;
        ...
        // Note: Didn't ACCOUNT FOR presence of discounts of BOTH `product_discount` (in `products` table) AND `category_discount` (in `categories` table) AT THE SAME TIME!!
            ...
            $final_price = $product['price_per_pax'] * $size;
            $discount = 0;
        }


        return array(
            'product_price' => $product['price_per_pax'] * $size, // the original price of that `product_id` and `size` in `products_attributes` table

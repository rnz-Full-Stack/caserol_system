...
class ProductsController extends Controller
{
    ...
    public function cartAdd(Request $request)
    {
        if ($request->isMethod('post')) { // if the Add to Cart <form> is submitted
            ...
            if ($eventDate > 0) {
                ...
                if ($data['addon_id']) {
                    foreach ($data['addon_id'] as $key => $addon_id) {
                        
                        CartAddon::where('cart_id', $item->id)->where('addon_id', $addon_id)->count();
    ...
    // Checkout page (using match() method for the 'GET' request for rendering the front/products/checkout.blade.php page or the 'POST' request for the HTML Form submission in the same page) (for submitting the user's Venue Address and Payment Method))
    public function checkout(Request $request)
    {
        ...
        $deliveryAddresses = \App\Models\DeliveryAddress::deliveryAddresses(); // the Venue Addresses of the currently authenticated/logged in user


        // Calculating the Shipping Charges of every one of the user's Venue Addresses (depending on the 'country' of the Venue Address)
        foreach ($deliveryAddresses as $key => $value) {
            $shippingCharges = \App\Models\ShippingCharge::getShippingCharges($total_weight, $value['country']);

            // Append/Add the Shipping Charge of every Venue Address (depending on the 'country' of the Venue Addresss) to the $deliveryAddresses array
            ...
            // Check if the COD PIN code of that Venue Address of the user exists in `cod_pincodes` table
            ...
            // Check if the Prepaid PIN code of that Venue Address of the user exists in `prepaid_pincodes` table
        ...
        if ($request->isMethod('post')) { // if the <form> in front/products/checkout.blade.php is submitted (the HTML Form that the user submits to submit their Venue Address and Payment Method)
            $data = $request->all();
            ...
            if (empty($data['address_id'])) { // if the user doesn't select a Venue Address
                $message = 'Please select Venue Address!';

                return redirect()->back()->with('error_message', $message);
            }

            // Payment Method Validation
            if (empty($data['payment_gateway'])) { // if the user doesn't select a Venue Address
            ...
            if (empty($data['accept'])) { // if the user doesn't select a Venue Address
            ...
            // Note: For the Orders module, we created two database tables: orders and orders_products tables. The first one holds/stores the main information about the orders of a user (e.g. Venue Address, coupon code, shipping, payment method, ...etc), and the second one holds/stores the detailed information about the order (the items/products that are bought by the order and product name, code, color, size, price, ...etc). There is a one-to-many relationship between the two tables where one order can have many order products.


            // Now, we'll collect the necessary data to fill in the `orders` and `orders_products` database tables

            // Get the Venue Address from    $data['address_id']
            ...
            // Get the Shipping Charge based on the chosen Venue Address
            ...
            foreach ($getCartItems as $item) {
                ...
                \App\Models\Product::where([ // Update the new `quantity` in the `products_attributes` table
                    'id' => $item['product_id'],
                 //   'size' => $item['size']
    ...
    // PIN code Availability Check: check if the PIN code of the user's Venue Address exists in our database (in both `cod_pincodes` and `prepaid_pincodes`) or not in front/products/detail.blade.php via AJAX. Check front/js/custom.js

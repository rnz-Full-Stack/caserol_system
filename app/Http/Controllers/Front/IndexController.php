...
class IndexController extends Controller
{
    public function index() {
        ...
        $newProducts   = \App\Models\Product::orderBy('id', 'Desc')->where('status', 1)->limit(8)->get()->toArray(); // show the LATEST (DESCendingly) 8 added products (to show the 'New Catering Setup' at the home page)    // Ordering, Grouping, Limit & Offset: https://laravel.com/docs/9.x/queries#ordering-grouping-limit-and-offset
        $bestSellers   = \App\Models\Product::where([
            'is_bestseller' => 'Yes',
            'status'        => 1 // product is enabled (active)
        ])->inRandomOrder()->get()->toArray(); // show the 'BestSeller' products with RANDOM ORDERING: https://laravel.com/docs/9.x/queries#random-ordering    // using the inRandomOder() method    // Only 'superadmin' can mark a product as 'Best Setup', but 'vendor' can't
        ...
        $meta_title       = 'Caserole Website';
        $meta_description = 'Online Multiple Catering Reservation Website which deals in Catering Services';
        $meta_keywords    = 'Online Catering Services, Caserole';

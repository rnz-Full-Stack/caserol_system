...
{
    // We got two Shipping Charges modules: Simple one (every country has its own shipping rate (price/cost/charges) based on the Venue Address in the Checkout page) and Advanced one (every country has its own shipping rate based on the Venue Address in the Checkout page, plus, other charges are calculated based on shipment weight). We created the `shipping_charges` database table for that matter. Also, the Shipping Charge module will be available in the Admin Panel for 'admin'-s only, not for 'vendor'-s

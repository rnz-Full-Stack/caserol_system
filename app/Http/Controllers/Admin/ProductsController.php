...
class ProductsController extends Controller
{
    ...
    public function addEditProduct(Request $request, $id = null) { // If the $id is not passed, this means 'Add a Product', if not, this means 'Edit the Product'    
        ...
        if ($request->isMethod('post')) { // WHETHER 'Add a Product' or 'Update a Product' <form> is submitted (THE SAME <form>)!!
            ...
            $product->description      = nl2br($data['description']);
            $product->details          = nl2br($data['details']);
    ...
    public function addAttributes(Request $request, $id) { // Add/Edit Attributes function    
        ...
        $product = \App\Models\Product::select('id', 'product_name', 'product_code', 'product_color', 'product_price', 'product_image', 'price_per_pax', 'pax', 'stock')->with('attributes')->find($id); // with('attributes') is the relationship method name in the Product.php model

        if ($request->isMethod('post')) { // When the <form> is submitted
            $data = $request->all();
            
            Product::where('id', $id)->update(['price_per_pax' => $request->price_per_pax, 'pax' => $request->pax, 'stock' => $request->stock]);
            // dd($data);

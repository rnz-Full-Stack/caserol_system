...
$(document).ready(function() {
    ...
    $(document).on('keyup', '#input_pax', function() {
       
        let pax = $(this).val();
        let total = pax * $('#price_per_pax').val();

        $('#orig_price').val(total.toFixed(2));
        $('.orig_price_text').text(total.toFixed(2))
        ...
        $('#discounted_price').val(total.toFixed(2));

        let addons = 0;
        $('#addonTable').find('.total').each(function(i, obj) {
            addons += parseFloat($(obj).val().replace('P', ''));
        });
        ...
        $('#withAddonTotal').text(total.toFixed(2));
    });
